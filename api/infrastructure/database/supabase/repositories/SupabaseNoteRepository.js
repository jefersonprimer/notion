...
exports.SupabaseNoteRepository = void 0;
...
class SupabaseNoteRepository {
    ...
    async findTopLevelByUserId(userId) {
        const { data, error } = await client_1.supabase
            .from('notes')
            .select('*')
            .eq('user_id', userId)
    ...
    async findByUserId(userId, sortBy = 'updated_at', sortDirection = 'desc') {
        const { data, error } = await client_1.supabase
            .from('notes')
            .select('*')
            .eq('user_id', userId)
    ...
    async update(id, userId, data) {
        const { data: updatedData, error } = await client_1.supabase
            .from('notes')
            .update(data)
    ...
    async search(userId, query, titleOnly, sortBy, sortDirection) {
        let filter;
        if (titleOnly) {
...
exports.SupabaseNoteRepository = SupabaseNoteRepository;
//# sourceMappingURL=SupabaseNoteRepository.js.map